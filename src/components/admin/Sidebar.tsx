...
export function Sidebar() {
  ...
  // Dynamically update the nav items with the actual order count
  const navItems = useMemo(() => {
    return ADMIN_NAV_ITEMS.map((item) => ({
      ...item,
      badge: item.label === "Orders" ? orders.length : item.badge,
    }));
  }, [orders.length]);
  ...
  return (
    ...
    >
      {/* Logo + Menu Icon */}
      <div
        className={`p-4 border-b border-gray-200 relative z-10 ${collapsed ? "flex flex-col items-center justify-center gap-2" : "flex items-center justify-between"}`}
            ...
            return (
              <Tooltip key={item.href}>
                ...
                {collapsed && (
                  <TooltipContent
                    side="right"
                    className="bg-gray-900 text-white px-2 py-1 rounded shadow"
                  >
                    {item.label}
                  </TooltipContent>
                )}
